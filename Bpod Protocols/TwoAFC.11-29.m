...
if isempty(fieldnames(S))       % If the settings file is empty, load the following default settings
    ...
    S.GUI.MinCenterPoke = 0.150;  % Minimum length of initiation poke
...
for currentTrial = 1:S.GUI.MaxTrials
    ...
    switch TrialTypes(currentTrial)         % Define which sound numbers do what.
        case 1
            LeftResponse = 'Timeout'; RightResponse = 'Reward'; RewardValve = 4; RewardTime = RightValveTime; LightCode= 'PWM3';
...
%             LeftResponse = 'Reward'; RightResponse = 'Timeout'; RewardValve = 3; RewardTime = RightValveTime; LightCode= 'PWM1';
        case 6
            LeftResponse = 'Reward'; RightResponse = 'Timeout'; RewardValve = 1; RewardTime = LeftValveTime; LightCode= 'PWM1';
    ...
    sma = AddState(sma, 'Name', 'WaitMinPokeTime', ...                                      % With nose in port 2, wait for rat to hold long enough (set by S.GUI.MinCenterPoke) and go to WaitForPoke
        ...
        'StateChangeConditions', {'Port2Out', 'exit', 'Tup', 'WaitForResponse'}, ...
        'OutputActions', {'BNCState', 1});
%     sma = AddState(sma, 'Name', 'CenterReward', ...             % Reward at center port after holding long enough; move on to WaitForResponse
%         'Timer', 0.1, ...
...
%         'OutputActions', {'WireState', 1});
    sma = AddState(sma, 'Name', 'WaitForResponse', ...                                      % After triggering a stimulus, wait for rat to poke left or right, and go to corresponding state.
        ...
        'OutputActions', {'WireState', RewardValve, LightCode, 255});
    sma = AddState(sma, 'Name', 'Reward', ...                                           % If correct response, send reward to that port.
        'Timer', 0.5, ...                                           % Change 0.5 to RewardTime when using solenoids
        'StateChangeConditions', {'Tup', 'Drinking'}, ...
        'OutputActions', {});               % Use this line when pumps are attached at wire outputs 1 and 3
    ...
    sma = AddState(sma, 'Name', 'Drinking', ...                                             % After reward, give them some time to drink without exiting.
        'Timer', .2, ...
        'StateChangeConditions', {'Tup', 'exit', 'Port1Out', 'ConfirmPortOut', 'Port3Out', 'ConfirmPortOut',}, ...
        'OutputActions', {});
    sma = AddState(sma, 'Name', 'ConfirmPortOut', ...                                       % confirm finished drinking.
        'Timer', .1, ...
...
Stimulus = squeeze(Stimuli(StimTable(TrialTypes(currentTrial+1),1),StimTable(TrialTypes(currentTrial+1),2),FileID,:))';
    ...
    if ~isnan(BpodSystem.Data.RawEvents.Trial{currentTrial}.States.WaitForResponse(1))
        S.GUI.MinCenterPoke = S.GUI.MinCenterPoke + 0.0001;
    end
